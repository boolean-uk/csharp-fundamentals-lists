...
namespace csharp_fundamentals_lists.Main
{
    public class Core
    {
        ...
        public List<string> Question3()
        {
            ...
            //TODO: you can 'chain' methods on the _iceCream list, so add another Concat to include EvenMoreIceCream  (this is defined below)
            //to the result list . e.g.   _iceCreams.Concat(this.MoreIceCream).Concat(other list to concat).ToList()
        ...
        public List<string> Question4()
        {
            ...
            // remove exception and write code here
            List<string> resultsNoDupl = _iceCreams.Concat(this.MoreIceCream).Concat(this.EvenMoreIceCream).Distinct().ToList();

            return resultsNoDupl;
        }
        bool pause = true;
